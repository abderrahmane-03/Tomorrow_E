...
<div class="flex justify-between">
    <form class="ml-10" action="{{ route('event-filtre') }}" method="post">
        ...
        <button class="bg-pink-300 text-pink-500 inline-flex p-2 ml-2 rounded-lg" type="submit"><svg class="w-[18px] h-[18px]  dark:text-pink-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    My tickets</a>
       <form class="mr-10" action="{{ route('event-search') }}" method="post">
        ...
        <button class="bg-violet-300 text-violet-500 inline-flex p-2 ml-2 rounded-lg" type="submit"><svg class="w-[18px] h-[18px] dark:text-violet-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="shadow p-4 rounded-lg bg-yellow-100 ">
                ...
                <div class="flex justify-between items-end mt-8">
                    <p class="text-lg font-semibold text-gray-800">
                        Organizer:{{$event->organisateur->user->name}}
                    </p>
                    @if ($event->type_of_reservation === "automatique" && $event->places_available > 0)
                    <form method="POST" action="{{ route('automatique.reservation', ['event' => $event->id]) }}">
                        @csrf
                    ...
                    @if ($event->type_of_reservation === "manuelle" && $event->places_available > 0)
                    <form action="{{route('manuelle.reservation',$event->id)}}" method="POST">
                        @csrf
